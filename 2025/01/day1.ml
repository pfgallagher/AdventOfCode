...
let determine_movement direction count =
  if direction = 'L' then -count else count

let count_crosses old_setting new_setting movement =
  if movement > 0 then new_setting / 100
  else if new_setting > 0 then 0
  else if old_setting > 0 then 1 + (new_setting / -100)
  else new_setting / -100
...
let rec loopP2 ic setting zero_count =
  ...
  | line ->
      ...
      loopP2 ic (dial_mod new_setting)
        (zero_count + count_crosses setting new_setting movement)
  | exception End_of_file -> zero_count
